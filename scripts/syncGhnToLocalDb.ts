...
import pool from '../src/config/db'; // Đảm bảo đường dẫn này đúng
...
const syncProvinces = async () => {
    ...
    const { data } = await axios.get(`${GHN_API_BASE}/master-data/province`, { headers });
    ...
    for (const province of data.data) {
        await pool.query(
            ...
            [province.ProvinceID.toString(), province.ProvinceName, province.ProvinceName]
        );
    }
...
const syncDistricts = async () => {
    ...
    const provinces = await pool.query('SELECT code FROM provinces');
    let totalDistricts = 0;

    for (const province of provinces.rows) {
        const provinceCode = province.code;
        const { data } = await axios.get(`${GHN_API_BASE}/master-data/district?province_id=${provinceCode}`, { headers });
            ...
            for (const district of data.data) {
                await pool.query(
                    `INSERT INTO districts (code, name, full_name, province_code) 
                     VALUES ($1, $2, $3, $4) 
                     ON CONFLICT (code) DO NOTHING`,
                    [district.DistrictID.toString(), district.DistrictName, district.DistrictName, provinceCode]
                );
                totalDistricts++;
            }
        }
...
const syncWards = async () => {
    ...
    for (const district of districts.rows) {
        ...
        try {
            const { data } = await axios.get(`${GHN_API_BASE}/master-data/ward?district_id=${districtCode}`, { headers });
                ...
                for (const ward of data.data) {
                    await pool.query(
                        `INSERT INTO wards (code, name, full_name, district_code) 
                         VALUES ($1, $2, $3, $4) 
                         ON CONFLICT (code) DO NOTHING`,
                        [ward.WardCode, ward.WardName, ward.WardName, districtCode]
                    );
                    totalWards++;
                }
            }
...
const runSync = async () => {
    try {
        ...
        await pool.end(); // Đóng kết nối DB để script kết thúc
    }
};
