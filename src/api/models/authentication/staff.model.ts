...
export const findStaffById = async (id: number) => {
    const result = await pool.query(`
        ...
        LEFT JOIN roles r ON u.role_id = r.id
        LEFT JOIN branches b ON u.branch_id = b.id
        WHERE u.id = $1 AND u.user_type != 2 AND u.deleted_at IS NULL
    `, [id]);
...
export const updateStaff = async (id: number, data: Partial<Omit<User, 'id' | 'password'>>) => {
    ...
    const result = await pool.query(
        `UPDATE users SET ${fields}, updated_at = NOW() WHERE id = $${values.length + 1} AND user_type != 2 RETURNING id, name, email, role_id, branch_id, status`,
        [...values, id]
    );
    return result.rows[0] || null;
};
...
export const softDeleteStaff = async (id: number): Promise<boolean> => {
    const result = await pool.query(
        'UPDATE users SET deleted_at = NOW(), status = 2 WHERE id = $1 AND user_type != 2 AND deleted_at IS NULL',
        [id]
    );
